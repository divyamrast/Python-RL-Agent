...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        critic_layer1 = tflearn.fully_connected(inputs, 400, activation='relu',
        ...
        critic_layer2 = tflearn.fully_connected(critic_layer1, 300,
                                                weights_init=tflearn.initializations.uniform(
                                                    minval=-1 / math.sqrt(400 + self.a_dim),
                                                    maxval=1 / math.sqrt(400 + self.a_dim)),
                                                regularizer='L2', weight_decay=0.01)
        critic_layer3 = tflearn.fully_connected(action, 300,
                                                weights_init=tflearn.initializations.uniform(
                                                    minval=-1 / math.sqrt(400 + self.a_dim),
                                                    maxval=1 / math.sqrt(400 + self.a_dim)),
                                                regularizer='L2', weight_decay=0.01)
